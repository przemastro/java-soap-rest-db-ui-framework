...
import com.company.qa.utils.WebEventListener;
...
public class UiTest {
    ...
    protected void afterClass(Scenario scenario) throws IOException {
        if (Configuration.driver != null && scenario.isFailed()) {
        ...
        Configuration.driver.manage().deleteAllCookies();
    }
    ...
    private void saveScreenshot(Scenario scenario, String destinationDir) {
        ...
        try {
            final byte[] screenshot = WebEventListener.takeScreenshot(Configuration.driver);
    ...
    private void saveBrowserLogs(String destinationDir) throws IOException {
        final Logs logs = Configuration.driver.manage().logs();
